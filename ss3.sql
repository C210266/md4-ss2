...
select px.MaKH,month(px.NgayBan), sum(ctpx.SoLluong*ctpx.GiaBan) from khachhang kh join phieuxuat px on kh.MaKH = px.MaKH join ctphieuxuat ctpx on px.sopx = ctpx.sopx
group by px.MaKH,month(px.NgayBan);

-- 12.
select  sp.MaSp, month(NgayBan),sum(ctpx.SoLluong) from sanpham sp join ctphieuxuat ctpx on sp.MaSP = ctpx.MaSP join phieuxuat px on ctpx.SoPX = px.SoPX 
group by sp.MaSp, month(NgayBan);

-- 16. 

select nv.MaNV, nv.HoTen, ctpx.MaSp, sum(ctpx.SoLluong * ctpx.GiaBan) 'doanh thu' from nhanvien nv join phieuxuat px on nv.MaNV = px.MaNV join ctphieuxuat ctpx on ctpx.SoPx = px.SoPX join sanpham sp on sp.MaSp = ctpx.MaSp group by nv.MaNV,nv.HoTen, ctpx.MaSp;
